## 목차
1. [플립플롭](#플립-플롭이란---메모리)
2. [bitlatch](#bit-latch)
3. [RAM](#ram-random-access-memory)
4. [Accumulator](#accumulator가산기)
5. [메모리계층](#메모리-계층---컴퓨터-구조-이해)
...
- Clear 입력 추가. (data 삭제)
    ...
    - Flip-flop 값을 삭제할 수 있다.
    - Data에 어떤 입력이 오든간에 Clear 입력으로 output 값 삭제

![](img/2022-04-15-10-25-04.png)

- 8-Bit Adder와 8 Bit Latch 로 구성
- 8-Bit Latch에서
    - Clear 스위치는 Clear 입력에 들어감
    - Add 스위치는 Clock 입력에 들어감
        - 8-bit Adder에서 덧셈이 계산되고, 이 값이 8-bit latch에 들어가려면, Add 스위치가 눌려졌을 때, 8-bit latch에 저장됨.

- 덧셈 계산을 해보자
    - Clear를 1로 만들어 기존 값을 삭제
    - Switches A 데이터를 넣고, Add를 누름
        - B는 현재 아무 값이 없는 상태
        - 결국 A의 데이터는 B에 들어감
    - Switches로 다시 다른 데이터를 A에 넣고, Add를 누름 -- > 결국 B = A + B

> 계산 하기 (A + B) 이후 (C + D)
>
$ 1. Switches(A) => 8_{Bit}Adder_{A} \;Input (A) 
\\2. 8_{Bit}Adder_{A} = A,\; 8_{Bit}Adder_{B} = 0
\\3. 8_{Bit}Adder_{output}(A) => 8_{Bit}Latch_{Input} = A
\\4. 8_{Bit}Latch_{Add}(on) => 8_{Bit}Latch_{value} = 8_{Bit}Latch_{Input}(A)
\\5. 8_{Bit}Latch_{output}(A) = 8_{Bit}Adder_{B}
\\6. Switches(B) => 8_{Bit}Adder_{A} = B
\\7. 8_{Bit}Adder_{A}(B), 8_{Bit}Adder_{B}(A)
=> 8_{Bit}Adder_{output}(A+B)
\\8. 8_{Bit}Latch_{Add}(on) => 8_{Bit}Latch_{Input}(A+B)
\\9. 8_{Bit}Latch_{output}(A+B)
\\(C+D)의 계산이 필요함
\\10. 8_{Bit}Latch_{claer}(on) => 8_{Bit}Latch_{value}(delete)
\\11. 위의 계산 과정과 동일(C+D)의 계산$
...
> CPU가 데이터를 읽어오는 순서
>
> 레지스터 => L1 캐쉬 => L2 캐쉬 => L3 캐쉬 => 메모리 => SSD => HDD
>
> CPU가 데이터를 읽어오는 흐름과 같이 걸리는 시간도 그와 비례해서 움직인다.
![](img/2022-04-15-10-33-36.png)
